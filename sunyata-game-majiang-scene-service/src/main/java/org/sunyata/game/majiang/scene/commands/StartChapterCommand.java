 ...
 */
@ServerCommand(commandId = Commands.gameChapterStart, routeModel = ServerCommand.directMode)
public class StartChapterCommand extends AbstractCommandHandler implements ApplicationContextAware {

    Logger logger = LoggerFactory.getLogger(StartChapterCommand.class);
...
//                .getGatewayServerId());
        ChapterStartMsg gameChapterStart = new ChapterStartMsg();
//        RoomImpi room = sceneUser.getRoom();
//        room.joinGame(sceneUser);
//        room.chapterStart(sceneUser);
        Object bean = applicationContext.getBean(Commands.gameChapterStart);
        roomService.handler((MessageHandler) bean, gameChapterStart, userIdInGateway, gatewayServerId);
    }
