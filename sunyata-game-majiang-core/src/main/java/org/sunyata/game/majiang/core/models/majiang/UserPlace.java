...
//    public UserPlace setOuting(Pai outing) {
//        this.outing = outing;
//        return this;
//    }

    private Pai outing = null;
    ...
    public UserPlaceMsg toMessage(boolean isMy, int shouIndex) {
        ...
        if (isMy) {
            ...
            m.setShouPai(MajiangUtils.toIndexForHide(shouPai.values()));
            m.setShouPaiLen(shouPai.size());
        }
     ...
     * 碰了的牌，自己在摸到一张，那么可以杠，叫小明杠
     */
    public List<Pai> checkXiaoMingGang(Pai pai) {
        //return peng.stream().filter(shouPaiMap::containsKey).mapToInt(Pai::getIndex).toArray();
        List<Pai> collect = peng.stream().filter(shouPaiMap::containsKey).collect(Collectors.toList());
        return collect;
    }
...
//    public ArrayList<Pai> checkTingPai(boolean isHuihuiGang, Pai pai, Pai huiEr[]) {
//        ArrayList<Pai> tingPais = new ArrayList<>();
//        if (isHuiErGang(isHuihuiGang, huiEr)) {
//            Collections.addAll(tingPais, huiEr);
//        }
...
//    }
    ...
    public boolean isQingYiSe() {
        if (shouPai.keySet().size() == 1) {
            ...
            return !type.isZiPai() && testAllPaiType(type);
        }
        return false;
    }
