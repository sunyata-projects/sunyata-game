...
public class JoinGameCommand extends AbstractCommandHandler {
    ...
    public void execute(OctopusPacketRequest request, OctopusPacketResponse response) throws Exception {
        ...
        if (userRoom != null) {
            ...
            if (roomEntity != null) {
                JoinGameToSceneMsg joinGameToSceneMsg = new JoinGameToSceneMsg().setGatewayServerId(gatewayServerId)
                        .setUserId(userRoom.getUserId()).setUserIdInGateway(userIdInGateway);
                anyClientManager.sendSysMessageByInnerGateway(Commands.joinGameToScene, joinGameToSceneMsg, roomEntity
                        .getSceneId());
            }
        }
    }
